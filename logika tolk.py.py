...
class Window(CTk):
    def __init__(self):
        ...
        self.text_menu.pack(pady = 20)
        self.name_menu = CTkEntry(self.menu)
        self.name_menu.pack()
        self.knopka = CTkButton(self.menu, text = "Підключитися", command=self.connect_to_server)
        ...
        self.chatik.place(x = 0, y = -200)
        self.massage = CTkEntry(self,placeholder_text="Ваше повідомлення")
        self.massage.place(x = 0, y = -200)
        self.send_message = CTkButton(self, text = '➢', width= 30, height= 30, command = self.sand_msg)
        self.adaptive()
    def connect_to_server(self):
        self.name = self.name_menu.get()
        try:
            self.client_socket = socket(AF_INET, SOCK_STREAM)
# це поле для введення айпі та логіну бо я дурак потеряюсь              
    ...
    def adaptive(self):
        ...
        self.chatik.configure(width = self.winfo_width()- self.menu.winfo_reqwidth(), height = self.winfo_height()- self.massage.winfo_height()- 20)
        self.chatik.place(x = self.menu.winfo_width(), y = 25)
        self.massage.configure(width = self.winfo_width()- self.menu.winfo_width()- 200, height = 30)
        self.massage.place(x = self.menu.winfo_width(), y = self.winfo_height()- 220)
        self.send_message.configure(width = self.massage.winfo_height()- 3, height = self.massage.winfo_height()- 4)
        self.send_message.place(x = self.winfo_width()- 200, y = self.winfo_height()- 222)
